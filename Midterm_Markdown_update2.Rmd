...
ggplot() + 
 ...
 geom_sf(data = NCData.sf, aes(colour = grocery_nn1)) + 
...
## Home Features Correlation #1 - percent heated
...
## Home Features Correlation #2 - building grade
st_drop_geometry(NCData.sf) %>% 
...
## Home Features Correlation #3 - Grocery KNN
st_drop_geometry(NCData.sf) %>% 
...
## Home Features Correlation #4 - Capital Improvement KNN
st_drop_geometry(NCData.sf) %>% 
  dplyr::select(price, percentheated) %>%
  filter(price <= 1000000) %>%
  gather(Variable, Value, -price) %>% 
